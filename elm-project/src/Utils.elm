...
getAngleBetweenTwo2DVectors v1 v2 =
    let
        dp =
            dotProductV2 v1 v2 |> Debug.log "dot-product"

        m1 =
            magnitudeV2 v1 |> Debug.log "magnitude of v1"

        m2 =
            magnitudeV2 v2 |> Debug.log "magnitude of v2"
...
findActivePanelEntry model =
    case model.activePlotEntry of
        Just activeExpression ->
            findPanelEntry model activeExpression
...
generalVectorToVector2D vector =
    case vector of
        ...
        _ ->
            Vec2D 0 0 |> Debug.log ("ERROR: generalVectorToVector2D invoked for a GeneralVector of length " ++ (vector |> List.length |> String.fromInt) ++ ".  Was expecting 2 items only")
...
generalVectorToVector3D vector =
    case vector of
        ...
        _ ->
            Vec3D 0 0 0 |> Debug.log ("ERROR: generalVectorToVector3D invoked for a GeneralVector of length " ++ (vector |> List.length |> String.fromInt) ++ ".  Was expecting 3 items only")
...
isActivePlotPanel model panelEntry =
    case model.activePlotEntry of
        ...
        Just expression ->
            panelEntry.expression == expression
...
applyFunctionToPanelEntryWithExpression expression func model =
    case findPanelEntry model expression of
        ...
        Nothing ->
            let
                _ =
                    Debug.log "ERROR: Cannot find panel with expression" expression
            in
            model
